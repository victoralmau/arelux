...
class ResPartnerSaleOrderFirstReport(models.Model):
    ...
    date_done_picking = fields.Datetime(
        string='Date done picking',
        readonly=True
    )
    ...
    def init(self):
        ...
        self._cr.execute("""
            CREATE VIEW res_partner_sale_order_first_report AS (
                ...
                LEFT JOIN (
                SELECT so2.partner_invoice_id,
                MIN(so2.confirmation_date) AS confirmation_date
                FROM sale_order AS so2
                WHERE so2.claim = FALSE AND so2.amount_untaxed > 0
                AND state in ('sale', 'done')
                GROUP BY so2.partner_invoice_id
                 ) AS so_1 ON so_1.partner_invoice_id = so.partner_invoice_id
                 LEFT JOIN (
                SELECT so2.partner_invoice_id, so2.id AS order_id,
                so2.confirmation_date, so2.name
                FROM sale_order AS so2
                GROUP BY so2.partner_invoice_id, so2.id
                 ) AS so_1_1 ON (
                so_1_1.partner_invoice_id = so.partner_invoice_id
                AND so_1_1.confirmation_date = so_1.confirmation_date)
                LEFT JOIN (
                SELECT sp.partner_id, MAX(sp.date_done) AS date_done, sp.origin
                FROM stock_picking AS sp
                WHERE sp.STATE  = 'done' AND sp.date_done IS NOT NULL
                AND sp.origin IS NOT NULL AND sp.picking_type_id = 4
                GROUP BY sp.partner_id, sp.origin
                ) AS sp_order_1 ON sp_order_1.origin = so_1_1.name
                WHERE so.claim = FALSE AND so.amount_total > 0
                AND STATE IN ('sale', 'done') AND order_id IS NOT NULL
