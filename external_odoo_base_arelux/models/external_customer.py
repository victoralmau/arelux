    ...
    def operations_item(self):
        ...
        if self.partner_id:
            if self.external_source_id:
                ...
                if self.external_source_id.external_customer_res_partner_category_id:
                    self.partner_id.category_id = [
                        (4, self.external_source_id.external_customer_res_partner_category_id.id)
                    ]
                # external_customer_res_partner_contact_form
                if self.external_source_id.external_customer_res_partner_contact_form:
                    if self.partner_id.ar_qt_activity_type == 'arelux':
                        self.partner_id.ar_qt_arelux_contact_form = [
                            (4, self.external_source_id.external_customer_res_partner_contact_form.id)
                        ]
