...
class ResPartnerReport(models.Model):
    ...
    last_date_order_management = fields.Datetime(
        srting='Date',
        readonly=True
    )
    total_sale_order_last_30_days = fields.Integer(
        string='Amount Untaxed Last 30 days',
        readonly=True
    )
    total_sale_order_last_90_days = fields.Integer(
        string='Amount Untaxed Last 90 days',
        readonly=True
    )
    total_sale_order_last_12_months = fields.Integer(
        string='Amount Untaxed Last 12 months',
        readonly=True
    )
    total_sale_order = fields.Integer(
        string='Total Sale Order More Than 300',
        readonly=True
    )
    ...
    def init(self):
        ...
        self._cr.execute("""
            CREATE VIEW res_partner_report AS (
                ...
                LEFT JOIN (
                SELECT partner_invoice_id, MAX(date_order_management)
                AS orders_date_order_management
                FROM sale_order
                WHERE claim = FALSE AND amount_total > 0
                AND STATE NOT IN ('draft', 'cancell')
                ...
                sum(amount_total) orders_amount
                FROM sale_order
                WHERE claim = FALSE AND amount_untaxed > 300
                AND state in ('sale', 'done')
                GROUP BY partner_invoice_id
