...
class ResPartner(models.Model):
    ...
    def cron_odoo_res_partner_fields_generate(self):
        self._cr.execute("""
            UPDATE res_partner SET (total_sale_order, total_sale_order_last_30_days,
            total_sale_order_last_90_days, total_sale_order_last_12_months,
            date_from_last_sale_order) = (
            SELECT rpr.total_sale_order, rpr.total_sale_order_last_30_days,
            rpr.total_sale_order_last_90_days, rpr.total_sale_order_last_12_months,
            rpr.last_date_order_management
            ...
            AND rpr2.last_date_order_management IS NOT NULL))
            LIMIT 1000
            )
        """)
        self._cr.execute("""
            UPDATE res_partner SET (account_invoice_amount_untaxed_total) = (
            SELECT ROUND((rpair.amount_untaxed_total_out_invoice-
            rpair.amount_untaxed_total_out_refund)::numeric,2)::FLOAT
            ...
            WHERE (rpair2.amount_untaxed_total_out_invoice IS NOT NULL
            OR rpair2.amount_untaxed_total_out_refund IS NOT NULL)
            AND (
        ...
        self._cr.execute("""
            ...
            WHERE ((rp.date_from_last_message IS NOT NULL
            AND rpmmr2.DATE  <> rp.date_from_last_message)
            OR (rp.date_from_last_message IS NULL
            AND rpmmr2.DATE IS NOT NULL))
            LIMIT 1000
            )
        """)
