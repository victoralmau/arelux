    ...
    def function_custom_user_id(self):
        ...
        for item in self:
            if item._origin and item.user_id:
                ...
                # user_id
                if item.partner_id:
                    if item.partner_id.user_id.id == 0 or not item.partner_id.user_id.id:
    ...
    def create(self, values):
        ...
        if values['ar_qt_customer_type'] != 'profesional':
            ...
            if 'date_deadline' not in values:
                allow_create = False
                raise UserError(
                    _('It is necessary to define an expected closing date to be able to create the flow')
                )
            else:                                                                                         
                if not values['date_deadline']:
                    ...
                    days_difference = (
                            fields.Datetime.from_string(values.get('date_deadline'))-current_date
                    ).days
                    if days_difference > 90:
                        allow_create = False
                        raise UserError(
                            _('The expected closure cannot be more than 90 days or prior to the current date (%s) when creating')
                            % days_difference
                        )
        # operations
        if allow_create:
            ...
            if self.ar_qt_customer_type != 'profesional':
                if 'date_deadline' in vals:
                    if not vals['date_deadline']:
                        ...
                        days_difference = (
                                fields.Datetime.from_string(vals['date_deadline'])-current_date
                        ).days
                        if days_difference > 90:
                            allow_write = False
                            raise UserError(
                    ...
                    if order_ids:
                        for order_id in order_ids:
                            if order_id.user_id.id == 0:
                                # update date_order
                                current_date = datetime.today()
                                order_id.date_order = current_date.strftime("%Y-%m-%d %H:%M:%S")
        # allow_write
        if allow_write:
