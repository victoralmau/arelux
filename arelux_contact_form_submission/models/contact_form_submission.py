...
class ContactFormSubmission(models.Model):
        ...
        if self.lead_id.id == 0 and self.partner_id.id > 0:
            ...
            if self.lead_id.id == 0:
                ...
                if self.next_activity_id.id>0:
                    crm_lead_vals['next_activity_id'] = self.next_activity_id.id
                # date_action
                if self.date_action>0:
                    date_action = current_date + relativedelta(days=self.date_action)
                    crm_lead_vals['date_action'] = str(date_action.strftime("%Y-%m-%d %H:%I:%S"))
                # lead_m2
                if self.m2>0:
                ...
                if len(sale_quote_template_id.quote_line) > 0:
                    ...
                    for quote_line in sale_quote_template_id.quote_line:
                        ...
                        if sale_quote_template_id.id == 3:  # Pto especial, cambiamos cosas
                            ...
                            # product_id
                            if quote_line.product_id.id in [70, 73]:  # Articulo extra o Cepillo
    ...
    @api.multi
    def cron_sqs_contact_form_submission(self, cr=None, uid=False, context=None):
